...
moviesForm.addEventListener("submit", function(e) {
    ...
    fetch(`https://www.omdbapi.com/?apikey=8dadc3aa&s=${input.value}`)
    .then(res=>res.json())
    .then(data=> {searchAllMovies(data.Search)})
})
...
function searchAllMovies(data) {
    if(data) {
        for(let movie of data) {
            fetch(`https://www.omdbapi.com/?apikey=8dadc3aa&t=${movie.Title}`)
                .then(res=>res.json())
